...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        try:
            obj = orm.Status.objects.get(name=u'новый')
        except ObjectDoesNotExist:
            orm.Status.objects.create(name=u'Новый', default=True)
        else:
            obj.default = True
            obj.name = u'Новый'
            obj.save()
        try:
            obj = orm.Status.objects.get(name=u'оплачено')
        except ObjectDoesNotExist:
            pass
        else:
            obj.name = u'Оплачено'
            obj.save()
        try:
            obj = orm.Status.objects.get(name=u'отправлено')
        except ObjectDoesNotExist:
            pass
        else:
            obj.name = u'Отправлено'
            obj.save()
        try:
            obj = orm.Status.objects.get(name=u'ошибка')
        except ObjectDoesNotExist:
            pass
        else:
            obj.name = u'Ошибка'
    ...
    models = {
        ...
        'basket.orderstatus': {
            ...
            'date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2010, 11, 24, 23, 22, 2, 779000)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
